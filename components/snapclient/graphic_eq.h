...
void eq_init(uint32_t sample_rate) {
    ...
    for (int band = 0; band < EQ_BANDS; band++) {
        ...
        if (eq.gain_db[band] == 0.0f) {
            ...
            // Note: ESP-DSP expects gain in linear scale (not dB)
            float gain_linear = powf(10.0f, eq.gain_db[band] / 20.0f);
            dsps_biquad_gen_peakingEQ_f32(eq.coeffs[band], norm_freq, eq.q_factor[band]);
            
            // Scale coefficients by gain (manual implementation since ESP-DSP doesn't have gain param)
            // For peaking EQ: multiply b0, b1, b2 by gain factor
